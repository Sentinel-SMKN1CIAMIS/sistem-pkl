<x-app-layout>
    ...
    <div class="mb-6">
        <p class="text-slate-600 dark:text-slate-400">Review performa dan tingkat keaktifan pembimbing sekolah dalam mengawal jurnal & kehadiran siswa PKL di program keahlian Anda secara real-time.</p>
    </div>
    ...
    <!-- Filters & Search -->
    <div class="glass-card p-6 mb-6" x-data="{ showFilters: {{ request()->anyFilled(['status', 'tipe']) ? 'true' : 'false' }} }">
        <form action="{{ route('kaprog.monitoring.index') }}" method="GET">
            <!-- Main Search Row -->
            <div class="flex flex-col md:flex-row items-stretch md:items-center gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    @if(request()->anyFilled(['search', 'status', 'tipe']))
                        <a href="{{ route('kaprog.monitoring.index') }}" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($mentors as $mentor)
            ...
            <div class="glass-card p-5 sm:p-6 flex flex-col justify-between border-t-2 {{ $mentor->activity_status === 'aktif' ? 'border-emerald-500/30' : ($mentor->activity_status === 'kurang_aktif' ? 'border-amber-500/30' : 'border-red-500/30') }}">
                ...
                <!-- Action Button -->
                <a href="{{ route('kaprog.monitoring.show', $mentor) }}" class="w-full py-2.5 flex items-center justify-center gap-2 text-xs font-bold bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl transition-all border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
