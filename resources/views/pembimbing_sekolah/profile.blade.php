    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 text-slate-700 dark:text-slate-300">
            ...
            <div class="glass-card p-8 text-center">
                <div class="relative inline-flex mb-6">
                    <div class="w-28 h-28 rounded-full overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl bg-slate-100">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($pembimbing->nama_lengkap) }}&background=8b5cf6&color=fff&size=200" 
                ...
                <p class="text-xs text-blue-500 font-bold uppercase tracking-wider mt-1">Pembimbing Sekolah</p>
                <p class="text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">{{ $pembimbing->nip ?: 'NIP: -' }}</p>
                
                <div class="mt-6 pt-6 border-t border-slate-200/50 dark:border-slate-700/50 flex flex-col gap-3">
                    <div class="flex items-center justify-between text-xs">
                        <span class="text-slate-500">Tipe Pembimbing</span>
                        <span class="font-bold text-slate-700 dark:text-slate-300 capitalize">{{ str_replace('_', ' ', $pembimbing->tipe) }}</span>
                    </div>
                    <div class="flex items-center justify-between text-xs">
                        <span class="text-slate-500">Konsentrasi Keahlian</span>
                        <span class="font-bold text-slate-700 dark:text-slate-300 text-right">{{ $pembimbing->konsentrasiKeahlian->nama ?? '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between text-xs">
                        <span class="text-slate-500">Kapasitas Bimbingan</span>
                        <span class="font-bold text-slate-700 dark:text-slate-300">{{ $pembimbing->kapasitas }} Siswa</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2">
            <div class="glass-card">
                ...
                <form action="{{ route('pembimbing_sekolah.profile.update') }}" method="POST" class="p-8 space-y-8">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="nip" class="block text-xs font-bold text-slate-500/80 dark:text-slate-400 uppercase mb-2">NIP</label>
                                <div class="relative">
                                    <input type="text" name="nip" id="nip" value="{{ old('nip', $pembimbing->nip) }}"
                                           placeholder="Contoh: 19820304..."
                                           class="w-full pl-10 pr-4 py-3 bg-slate-100 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all text-sm">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
