<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="glass-card p-6 border-t-4 border-blue-500">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center shrink-0">
        ...
        <div class="glass-card p-6 border-t-4 border-purple-500">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-purple-500/20 rounded-xl flex items-center justify-center shrink-0">
        ...
        <div class="glass-card p-6 border-t-4 border-emerald-500">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-emerald-500/20 rounded-xl flex items-center justify-center shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        ...
        <div class="glass-card p-6 border-t-2 border-slate-200/50 dark:border-slate-700/50">
            ...
            <div class="relative h-64 flex items-center justify-center">
                <canvas id="penempatanChart" 
                        data-total-siswa="{{ $stats['total_siswa'] ?? 120 }}"
                        data-total-dudi="{{ $stats['total_dudi'] ?? 45 }}"
                        data-total-pembimbing="{{ $stats['total_pembimbing'] ?? 15 }}"></canvas>
            </div>
        </div>
        ...
        <div class="glass-card p-6 border-t-2 border-slate-200/50 dark:border-slate-700/50">
            ...
            <div class="relative h-64 flex items-center justify-center">
                <canvas id="keaktifanChart"
                        data-week-jurnal='{!! json_encode($stats["week_jurnal"] ?? [0, 0, 0, 0, 0]) !!}'
                        data-week-absensi='{!! json_encode($stats["week_absensi"] ?? [0, 0, 0, 0, 0]) !!}'></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const canvasPenempatan = document.getElementById('penempatanChart');
            const totalSiswa = parseInt(canvasPenempatan.getAttribute('data-total-siswa')) || 0;
            const totalDudi = parseInt(canvasPenempatan.getAttribute('data-total-dudi')) || 0;
            const totalPembimbing = parseInt(canvasPenempatan.getAttribute('data-total-pembimbing')) || 0;
            const ctxPenempatan = canvasPenempatan.getContext('2d');
            new Chart(ctxPenempatan, {
                type: 'doughnut',
                data: {
                    labels: ['Siswa PKL', 'DUDI Terdaftar', 'Pembimbing'],
                    datasets: [{
                        data: [totalSiswa, totalDudi, totalPembimbing],
            ...
            // Keaktifan Chart (Bar)
            const canvasKeaktifan = document.getElementById('keaktifanChart');
            const weekJurnal = JSON.parse(canvasKeaktifan.getAttribute('data-week-jurnal') || '[0,0,0,0,0]');
            const weekAbsensi = JSON.parse(canvasKeaktifan.getAttribute('data-week-absensi') || '[0,0,0,0,0]');
            const ctxKeaktifan = canvasKeaktifan.getContext('2d');
