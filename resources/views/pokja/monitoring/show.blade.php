<x-app-layout>
    ...
    <div class="mb-6">
        <a href="{{ route('pokja.monitoring.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="glass-card p-4 text-center">
                    <p class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-widest mb-1">Total Siswa</p>
            ...
            <div class="glass-card overflow-hidden">
                <div class="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Periksa kinerja pembimbing dalam memproses jurnal/absensi untuk masing-masing siswa.</p>
                </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400 italic">
                                        Belum ada siswa yang ditugaskan kepada pembimbing sekolah ini.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="space-y-6">
            ...
            <!-- Log Input Form (Only for non-Kepala Sekolah users, i.e., Pokja and Super Admin) -->
            @if(auth()->user()->role !== 'kepala_sekolah')
                <div class="glass-card p-6">
                    ...
                    <form action="{{ route('pokja.monitoring.storeNote', $pembimbingSekolah) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Tingkat Evaluasi / Status</label>
                            <select name="status" required class="w-full px-4 py-2.5 bg-slate-100 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all text-xs font-semibold">
                                <option value="pending">Peringatan Ringan (Aktivitas Bimbingan Kurang)</option>
                                <option value="rejected">Teguran Keras / Penting (Mengabaikan Tugas)</option>
                                <option value="approved">Catatan Baik / Apresiasi (Menjalankan Tugas Dengan Benar)</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Catatan Evaluasi / Monitoring</label>
                            <textarea name="catatan" rows="4" placeholder="Tuliskan catatan monitoring pembimbing secara spesifik..." required
                                      class="w-full p-4 bg-slate-100 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all text-xs leading-relaxed">{{ old('catatan') }}</textarea>
                            @error('catatan')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <button type="submit" class="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-500/25 transition-all flex items-center justify-center gap-2">
