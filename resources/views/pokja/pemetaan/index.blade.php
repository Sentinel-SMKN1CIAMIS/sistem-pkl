<x-app-layout>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <p class="text-slate-400">Monitoring status penempatan siswa ke Industri dan Pembimbing.</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            <div class="px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                ...
                <p class="text-lg font-bold text-slate-100">{{ $terpetakan }} / {{ $totalSiswa }}</p>
            </div>
            <div class="px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                ...
                <p class="text-lg font-bold text-slate-100">{{ $totalSiswa - $terpetakan }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-800/50 border-b border-slate-700/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Tempat PKL (DUDI)</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Pembimbing Sekolah</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Pembimbing DUDI</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Status</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Aksi</th>
                    </tr>
                </thead>
                    ...
                    @forelse($siswas as $siswa)
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                <p class="text-[10px] text-slate-500 font-mono tracking-wider">{{ $siswa->nis }} | {{ $siswa->konsentrasiKeahlian->nama_konsentrasi ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                @if($siswa->dudi)
                                    <span class="text-slate-300 font-medium">{{ $siswa->dudi->nama_perusahaan }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 text-sm">
                                @if($siswa->pembimbingSekolah)
                                    <span class="text-slate-300">{{ $siswa->pembimbingSekolah->nama_lengkap }}</span>
                                @else
                                    <span class="text-slate-600 italic">Belum ditentukan</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm">
                                @if($siswa->pembimbingDudi)
                                    <span class="text-slate-300">{{ $siswa->pembimbingDudi->nama_lengkap }}</span>
                                @else
                                    <span class="text-slate-600 italic">Belum ditentukan</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <a href="{{ route('pokja.siswa.edit', $siswa->id) }}" class="inline-flex items-center justify-center p-2 rounded-lg bg-slate-800 border border-slate-700 hover:bg-slate-700 transition-colors text-slate-400 hover:text-blue-400">
