<x-app-layout>
    <x-slot name="header">Tambah Akun Kaprog</x-slot>
    ...
    <div class="max-w-4xl text-slate-700 dark:text-slate-300">
        <form action="{{ route('pokja.kaprog.store') }}" method="POST" class="space-y-6">
            @csrf
                        ...
                        <div>
                            ...
                            <select name="program_keahlian_id" id="program_keahlian_id" required
                                ...
                                @foreach($programs as $item)
                                    <option value="{{ $item->id }}" {{ old('program_keahlian_id') == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </select>
