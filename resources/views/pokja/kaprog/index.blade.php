                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <div x-data="{ open: false }" class="relative flex justify-end" x-on:click.away="open = false">
                                    ...
                                    <div x-show="open" 
                                        ...
                                        <form action="{{ route('pokja.kaprog.destroy', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun Kaprog {{ addslashes($item->user->name ?? '') }}?')">
                                            @csrf
                                            @method('DELETE')
