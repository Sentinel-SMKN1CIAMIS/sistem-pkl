    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="glass-card p-6">
            <div class="flex items-center gap-4 mb-2">
                ...
                <div>
                    <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest">Total Siswa</p>
                    <p class="text-2xl font-bold text-slate-100">{{ $totalSiswa }}</p>
                </div>
            </div>
        ...
        <div class="glass-card p-6">
            <div class="flex items-center gap-4 mb-2">
                ...
                <div>
                    <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest">Laporan Masuk</p>
                    <p class="text-2xl font-bold text-slate-100">{{ $laporanMasuk }}</p>
                </div>
            </div>
            <div class="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
        ...
        <div class="glass-card p-6">
            <div class="flex items-center gap-4 mb-2">
                ...
                <div>
                    <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest">Rata-rata Jurnal Valid</p>
                    <p class="text-2xl font-bold text-slate-100">{{ number_format($rataJurnal, 1) }}</p>
                </div>
            </div>
            <div class="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
        ...
        <form action="{{ route('pokja.evaluasi.index') }}" method="GET" class="flex gap-4">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama siswa atau NIS..." 
                       class="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border-slate-700/50 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-200">
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-800/50 border-b border-slate-700/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Progress Jurnal</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Kehadiran</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Laporan Akhir</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Status Evaluasi</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Rapor</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-700/50">
                    @forelse($siswas as $siswa)
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            <td class="px-6 py-4">
                                <p class="text-sm font-bold text-slate-200">{{ $siswa->nama_lengkap }}</p>
                                <p class="text-[10px] text-slate-500 font-mono tracking-wider">{{ $siswa->nis }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden min-w-[100px]">
                                        @php 
                                    ...
                                    <span class="text-xs font-bold text-slate-300">{{ $siswa->valid_jurnal }}/{{ $siswa->total_jurnal }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-3 py-1 rounded-lg bg-slate-800 border border-slate-700 text-xs font-mono text-slate-300">
                            ...
                            <td class="px-6 py-4 text-center">
                                <button class="p-2 rounded-lg bg-slate-800 border border-slate-700 hover:bg-slate-700 transition-colors text-slate-400 hover:text-blue-400">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-slate-500 font-medium">Belum ada data evaluasi yang tersedia.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
