<x-app-layout>
    <x-slot name="header">Kelola Buku Panduan</x-slot>

    <div class="mb-6 flex justify-between items-center">
        <p class="text-slate-400">Unggah dan kelola buku panduan untuk siswa, DUDI, atau umum.</p>
        <a href="{{ route('admin.panduan.create') }}" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl shadow-lg shadow-blue-500/25 transition-all flex items-center gap-2">
            <i data-lucide="upload-cloud" class="w-5 h-5"></i>
            Unggah Panduan
        </a>
    </div>

    @if(session('success'))
        <div class="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm flex items-center gap-3">
            <i data-lucide="check-circle" class="w-5 h-5"></i>
            {{ session('success') }}
        </div>
    @endif

    <div class="glass-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-800/30 border-b border-slate-700/50 text-slate-400 text-xs uppercase font-bold tracking-wider">
                        <th class="px-6 py-4">Judul Panduan</th>
                        <th class="px-6 py-4">Tipe / Sasaran</th>
                        <th class="px-6 py-4">Jurusan</th>
                        <th class="px-6 py-4">Tgl Upload</th>
                        <th class="px-6 py-4 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-700/50 text-sm">
                    @forelse($panduans as $item)
                        <tr class="hover:bg-slate-800/10 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="p-2 bg-red-500/10 rounded-lg">
                                        <i data-lucide="file-text" class="w-5 h-5 text-red-400"></i>
                                    </div>
                                    <span class="text-slate-200 font-medium">{{ $item->judul }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-0.5 rounded-full text-[10px] uppercase font-bold border border-slate-700 text-slate-400">
                                    {{ $item->tipe }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-slate-400">
                                {{ $item->konsentrasiKeahlian->nama ?? 'Semua Jurusan' }}
                            </td>
                            <td class="px-6 py-4 text-slate-500">
                                {{ $item->created_at->format('d M Y') }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" class="p-2 text-slate-400 hover:text-blue-400 transition-colors">
                                        <i data-lucide="eye" class="w-4 h-4"></i>
                                    </a>
                                    <form action="{{ route('admin.panduan.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus panduan ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button class="p-2 text-slate-400 hover:text-red-400 transition-colors">
                                            <i data-lucide="trash-2" class="w-4 h-4"></i>
                                        </button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500">Belum ada buku panduan yang diunggah.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</x-app-layout>
