<x-app-layout>
    ...
    <div class="max-w-4xl text-slate-300">
        <div class="mb-6">
            <p class="text-slate-400">Kelola pengaturan dasar aplikasi seperti Nama Sekolah, Tahun Ajaran, dan logo.</p>
        </div>
            ...
            <form action="{{ route('admin.config.update') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    @foreach($defaultConfigs as $item)
                        ...
                        @endphp
                        <div>
                            <label for="{{ $item['key'] }}" class="block text-sm font-black text-slate-500 uppercase tracking-widest mb-2">{{ $item['label'] }}</label>
                            <input type="text" name="{{ $item['key'] }}" id="{{ $item['key'] }}" value="{{ $value }}" placeholder="{{ $item['placeholder'] }}"
                                   class="w-full px-4 py-3 bg-slate-900/50 border border-slate-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 transition-all">
                        </div>
                    @endforeach
                </div>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10">
                ...
                <p class="text-[11px] text-slate-500 leading-relaxed mb-3">Lakukan backup database secara rutin untuk menghindari kehilangan data penting PKL.</p>
                <button class="text-xs font-bold text-slate-300 hover:text-white flex items-center gap-1 transition-colors">
            ...
            <div class="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                ...
                <p class="text-[11px] text-slate-500 leading-relaxed mb-3">Bersihkan data transaksi (absensi & jurnal) untuk persiapan tahun ajaran baru.</p>
