            ...
            <div class="glass-card p-6 text-center">
                ...
                @elseif($absensiToday->status === 'hadir' && !$absensiToday->waktu_pulang)
                    ...
                    <form action="{{ route('siswa.absensi.clock-out') }}" method="POST" id="clockout-form">
                        ...
                        <div id="alasan-container" class="mb-4 hidden">
                            <label class="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">Alasan Pulang Cepat (wajib diisi jika pulang sebelum 7 jam)</label>
                            <textarea name="alasan" id="alasan-input" rows="2" class="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-orange-500 focus:border-transparent"></textarea>
                        </div>
