                    ...
                    @if($laporan)
                        <div class="space-y-4">
                            ...
                            @if($laporan->file_path)
                                <a href="{{ asset('storage/' . $laporan->file_path) }}" target="_blank" class="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-700 dark:text-slate-300 transition-all">
                ...
                <div class="glass-card p-8">
                    ...
                    @else
                        <form action="{{ route('siswa.laporan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">File Laporan (PDF/Docx, Max 5MB)</label>
                                <div class="flex items-center justify-center w-full">
                                    <label for="file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-xl cursor-pointer bg-slate-100 dark:bg-slate-900/20 hover:bg-slate-100 dark:bg-slate-900/40 transition-all">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <i data-lucide="file-up" class="w-8 h-8 text-slate-500 dark:text-slate-400 mb-2"></i>
                                            <p class="text-xs text-slate-600 dark:text-slate-400">Pilih file laporan Anda</p>
                                        </div>
